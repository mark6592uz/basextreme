...
void ClauncherDlg::OnCbnSelchangeForest()
{
    // retrieve configuration element
    TiXmlElement* details = getConfigElement( "details" ); assert( details );    

    float option = float( m_Forest.GetItemData( m_Forest.GetCurSel() ) ) / 100.0f;
    char str[512];
    sprintf( str, "%3.2f", option );
    details->SetAttribute( "forest", str );
...
void ClauncherDlg::OnBnClickedShadows()
{
    // retrieve configuration element
    TiXmlElement* video = getConfigElement( "video" ); assert( video );

    // change settings
    int checkStatus = m_Shadows.GetCheck();
    if( checkStatus )
    {
        video->SetAttribute( "shadows", 1 );
    }
    else
    {
        video->SetAttribute( "shadows", 0 );
    }
...
void ClauncherDlg::OnBnClickedPitchShift()
{
    // retrieve configuration element
    TiXmlElement* sound = getConfigElement( "sound" ); assert( sound );

    // change settings
    int checkStatus = m_PitchShift.GetCheck();
    if( checkStatus )
    {
        sound->SetAttribute( "pitchShift", 1 );
    }
    else
    {
